...
# 
# grep -o "[a-zA-Z0-9.\-]\+@[.a-zA-Z0-9]\+\?\([0-9]\)\{2,4\}[ \-]\?\)\?[0-9]\{3\}\([ \-]\)\?[0-9]\{3\}\|" "${file}" | sort

# grep -o '\+\?\([0-9]\{2,4\}[ \-]\?\)\?[0-9]\{3\}\([ \-]\)\?[0-9]\{3\}\([ \-]\)\?[0-9]\{3\}\|' ${plik} | sort


# grep -o '\+\?\([0-9]\{2,4\}[ \-]\?\)\?[0-9]\{3\}\([ \-]\)\?[0-9]\{3\}\([ \-]\)\?[0-9]\{3\}\|[A-Za-z0-9.\-]\+@[A-Za-z0-9.]\+' ${file} | sort

# grep -o '[A-Za-z0-9.\-]\+@[A-Za-z0-9.]\|\+\?\([0-9]\{2,4\}[ \-]\?\)\?[0-9]\{3\}\([ \-]\)\?[0-9]\{3\}\([ \-]\)\?[0-9]\{3\}\+' ${file} | sort

grep -o "\|[a-zA-Z0-9.\-]\+@[.a-zA-Z0-9]\+\|\+\?\([0-9]\{2,4\}[ \-]\?\)\?[0-9]\{3\}\([ \-]\)\?[0-9]\{3\}\([ \-]\)\?[0-9]\{3\}" "${file}" | sort -u